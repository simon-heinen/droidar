...
public class StaticDemoSetup extends Setup {
	...
	private void initI9Tests(World w) {
...

		{
			...
			triangleMesh.addChild(new AnimationFaceToCamera(camera, 0.5f));
			triangleMesh.setScale(new Vec(10, 10, 10));
			GeoObj treangleGeo = new GeoObj(GeoObj.newRandomGeoObjAroundCamera(camera, MIN_DIST, MAX_DIST), triangleMesh);
			w.add(treangleGeo);
		}
...

		{
			...
			MeshComponent button = GLFactory.getInstance().newTexturedSquare("simpleUiId",
					IO.loadBitmapFromView(v, MeasureSpec.makeMeasureSpec(400, MeasureSpec.AT_MOST), LayoutParams.WRAP_CONTENT));
			button.setOnClickCommand(new CommandShowToast(getActivity(),
					"Thanks alot"));

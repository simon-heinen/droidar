...
public class DebugSetup extends Setup {
	...
	public void _b_addWorldsToRenderer(GL1Renderer renderer, GLFactory objectFactory, GeoObj currentPosition) {
		...
		world.add(objectFactory.newTextObject("text Input", new Vec(10, 1, 1), getActivity()/*myTargetActivity*/, camera));
	...
	private void initNTest(World w) {
		{
			MeshComponent triangleMesh = GLFactory.getInstance()
												  .newTexturedSquare(
														  "elefantId",
														  IO.loadBitmapFromId(getActivity()/*myTargetActivity*/, R.drawable.elephant64));
		...
		}
		{
			...
			MeshComponent triangleMesh = GLFactory.getInstance()
												  .newTexturedSquare(
														  "hippoId",
														  IO.loadBitmapFromId(getActivity()/*myTargetActivity*/, R.drawable.hippopotamus64));
			triangleMesh.addChild(new AnimationFaceToCamera(camera, 0.5f));
			triangleMesh.setScale(new Vec(10, 10, 10));
...

		{
			MeshComponent triangleMesh = GLFactory.getInstance()
												  .newTexturedSquare(
														  "pandaId",
														  IO.loadBitmapFromId(getActivity()/*myTargetActivity*/, R.drawable.panda64));
			triangleMesh.addChild(new AnimationFaceToCamera(camera, 0.5f));
			triangleMesh.setScale(new Vec(10, 10, 10));
	...
	private void initI9Tests(World w) {
...

		{
			...
			TextView tv = new TextView(getActivity()/*myTargetActivity*/);
			tv.setTextColor(Color.white().toIntARGB());
			tv.setTextSize(20);
			tv.setText("! Hallo !");

			Button b = new Button(getActivity()/*myTargetActivity*/);
			b.setText("Click Me");
			MeshComponent button = GLFactory.getInstance().newTexturedSquare(
					"buttonId", IO.loadBitmapFromView(b));
			button.setOnClickCommand(new CommandShowToast(getActivity()/*myTargetActivity*/, "Thanks alot"));
	...
	private synchronized void initWorld(RenderList l) {
		...
		MeshComponent treangleMesh = GLFactory.getInstance().newTexturedSquare(
				"worldIconId", IO.loadBitmapFromId(getActivity()/*myTargetActivity*/, R.drawable.icon));

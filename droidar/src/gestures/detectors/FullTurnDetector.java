...
/**
 * The FullTurnDetector can be used to determine whether the player has performed a
 * turn of roughly 360 degrees while holding the phone. Detection of this gesture is
 * based on sampling of the compass coordinates. The gesture allows a certain slowness
 * of the movement but resets itself if there has not been a large change in the
 * compass direction for more than 1.5s.
 *  
...
public class FullTurnDetector implements PhoneGestureDetector {
	...
	public double getProbability() {
		/*
		 * The probability of this event is the number of seen different readings.
		 */
		// Log.d("Full Turn Probability", Double.toString((double)this.seenDifferentReadings / NUMBER_OF_AZIMUTH_BINS));
	...
	public void feedSensorEvent(SensorData sensorData) {
		// Step 1: Get the orientation according to the new (API >= 8) method (see Sensor class documentation)
		SensorManager.getRotationMatrix(rotationMatrix, null, sensorData.gravity, sensorData.mag);
		SensorManager.getOrientation(rotationMatrix, orientationMatrix);
		
		/*
		 * Step 2: The current azimuth is given by the 1st entry in the orientation matrix; 
		 * we normalize it to the value range 0..360.
		 */
		...
		/*
		 * Step 3: Check if the player is facing the same direction for a longer period of time.
		 * In this case, reset the instance (there is no motion going on at the moment).
		 */
		...
		if((Math.abs(this.lastSeenCoarseOrientation - orientationIntCoarse) % NUMBER_OF_COARSE_BINS) <= 1) {
			if(System.currentTimeMillis() - this.timeOfLastSeenCoarseOrientationChange >= FullTurnDetector.TIME_BEFORE_RESET_IN_MS) {
				this.reset();
			}
		} else {
